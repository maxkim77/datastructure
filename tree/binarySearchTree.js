...
class BinarySearchTree {
    ...
    - insertNode: 새로운 노드를 삽입하는 메서드 : 새로운 노드의 값이 현재 노드의 값보다 작으면 왼쪽 자식 노드로, 크면 오른쪽 자식 노드로 이동
    - inOrderTraverseNode: 중위 순회를 구현하는 메서드 - 왼쪽 자식 노드, 현재 노드, 오른쪽 자식 노드 순으로 방문
    - inOrderTraverse: 중위 순회를 구현하는 메서드 - 중위 순회를 시작하는 메서드
    - searchNode: 특정 값을 검색하는 메서드 - 현재 노드가 null이면 false를 반환하고, 값이 현재 노드의 값보다 작으면 왼쪽 자식 노드로, 크면 오른쪽 자식 노드로 이동
